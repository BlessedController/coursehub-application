...
public class CourseRatingServiceImpl implements CourseRatingService {
    ...
    @Override
    public void rateCourse(String userId, CourseRatingRequest request) {

        Boolean body = courseServiceClient.isUserOwnerOfCourse(userId, request.courseId()).getBody();

        if (TRUE.equals(body)) {
            throw new InvalidRequestException("You are owner of this course");
        }

        if (courseRatingRepository.existsByUserIdAndCourseId(userId, request.courseId())) {
