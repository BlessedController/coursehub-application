...
import static com.coursehub.identity_service.config.RabbitMQConfig.*;
...
public class UserService implements IUserService {
    ...
    public boolean existsByEmail(String email) {
        log.debug("Entering existsByEmail with email: {}", email);
        boolean result = userRepository.existsByEmail(email);
        log.debug("Exiting existsByEmail -> {}", result);
        return result;
    }
    ...
    public void verify(UserPrincipal principal, String activationCode) {
        ...
        User userByActivationCode = userRepository.findByActivationCode(activationCode)
                .orElseThrow(() -> {
                    ...
                    return new UserNotFoundException("User with activation code " + activationCode + " not found");
                });
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(name = ADD_INSTRUCTOR_RATING_QUEUE, durable = "true"),
            exchange = @Exchange(name = EXCHANGE_NAME),
            key = ADD_INSTRUCTOR_RATING_ROUTING_KEY
    ))
    public void listenAddInstructorRating(RatingMQResponseForIdentityService response) {

        User instructor = findUserByIdAndUserStatusInAndUserRoleIn(response.instructorId(), Set.of(ACTIVE), Set.of(ROLE_INSTRUCTOR));

        addRating(response.rating(), instructor);
    ...
    @Override
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(name = DELETE_INSTRUCTOR_RATING_QUEUE, durable = "true"),
            exchange = @Exchange(name = EXCHANGE_NAME),
            key = DELETE_INSTRUCTOR_RATING_ROUTING_KEY
    ))
    public void listenDeleteInstructorRating(RatingMQResponseForIdentityService response) {

        User instructor = findUserByIdAndUserStatusInAndUserRoleIn(response.instructorId(), Set.of(ACTIVE), Set.of(ROLE_INSTRUCTOR));

        removeRating(response.rating(), instructor);
    ...
    private void addRating(Double rating, User instructor) {

        Double total = instructor.getRating() * instructor.getRatingCount();
        Double newAverage = (total + rating) / (instructor.getRatingCount() + 1);

        instructor.setRatingCount(instructor.getRatingCount() + 1);
        instructor.setRating(newAverage);
    }

    private void removeRating(Double rating, User instructor) {

        double total = instructor.getRating() * instructor.getRatingCount();

        if (instructor.getRatingCount() == 1) {
            instructor.setRatingCount(0L);
            instructor.setRating(0.0);
        } else if (instructor.getRatingCount() >= 2) {
            Double newAverage = (total - rating) / (instructor.getRatingCount() - 1);

            instructor.setRatingCount(instructor.getRatingCount() - 1);
            instructor.setRating(newAverage);
        } else {
            log.warn("Instructor {} has invalid ratingCount={}", instructor.getId(), instructor.getRatingCount());
        }
    }
